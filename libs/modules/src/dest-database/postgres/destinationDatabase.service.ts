...
export class DestinationDatabaseService implements IDestinationDatabaseService {
  ...
  async deleteData(dataSourceId: DataSourceId, dataTableName: string) {
    const data = await this.entityManager.transaction(async (entityManager) => {
      await entityManager.query(`DROP TABLE IF EXISTS "${dataTableName}"`);
      await entityManager.query(
        `DELETE FROM schemas WHERE data_source_id = '${dataSourceId}'`,
      );
    });
    return data;
  }
