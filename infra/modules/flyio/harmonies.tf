...
resource "null_resource" "fly_ipv4_rate_limiter" {
  count = local.rate_limiter_count
  triggers = {
    app = local.rate_limiter_app_name
  }
  provisioner "local-exec" {
    command = "flyctl ips allocate-v4 -a ${local.rate_limiter_app_name} -y -t $FLY_API_TOKEN"
  }
  depends_on = [
...
resource "null_resource" "rate_limiter_builder" {
  ...
  provisioner "local-exec" {
    ...
    working_dir = local.rate_limiter_path
  }
